...
class ANNNewsSource : NewsSource {
    ...
    override suspend fun getLatestNews(page: Int): List<NewsItem> = withContext(Dispatchers.IO) {
        try {
            val doc = Jsoup.connect("https://www.animenewsnetwork.com/newsfeed/")
                .userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
                .timeout(15000)
                .get()

            // ANN news feed is RSS-like HTML. Parse article entries
            doc.select("div.herald, div.wrap, div.mainfeed-section div.wrap a.herald").mapNotNull { item ->
                try {
                    val link = item.select("a[href]").first()
                    val title = item.select("h3, .herald-title, span").first()?.text()
    ...
    override suspend fun getNewsForMedia(title: String, malId: Int?): List<NewsItem> {
