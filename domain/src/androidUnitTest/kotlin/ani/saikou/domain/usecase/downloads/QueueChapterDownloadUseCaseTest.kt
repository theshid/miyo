...
class QueueChapterDownloadUseCaseTest {
    ...
    @Test
    fun `queues when no row exists`() = runTest {
    ...
    @Test
    fun `skips when row is QUEUED`() = runTest {
    ...
    @Test
    fun `skips when row is DOWNLOADING`() = runTest {
    ...
    @Test
    fun `skips when row is COMPLETED`() = runTest {
    ...
    @Test
    fun `re-queues a PAUSED row so resume works`() = runTest {
    ...
    @Test
    fun `re-queues an ERROR row so retry works`() = runTest {
    ...
    private fun mockDownload(status: DownloadStatus) = Download(
        id = downloadId,
